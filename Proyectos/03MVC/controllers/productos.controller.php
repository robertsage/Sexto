...
switch ($_GET["op"]) {
    ...
    case 'uno': // Procedimiento para obtener un producto por ID
        if (!isset($_POST["idProductos"]) || !isset($_POST["idKardex"])) {
        ...
        $datos = array();
        $datos = $producto->uno($idProductos, $idKardex);
    ...
    case 'actualizar': // Procedimiento para actualizar un producto existente
        ...
        $Proveedores_idProveedores = intval($_POST["Proveedores_idProveedores"]);
        $idKardex = intval($_POST["idKardex"]);
        // print_r("valor kardex en controller actualziar   " . $idKardex . "        . ");

        $datos = array();
        $datos = $producto->actualizar($idProductos, $Codigo_Barras, $Nombre_Producto, $Graba_IVA, $Unidad_Medida_idUnidad_Medida, $IVA_idIVA, $Cantidad,
        $Valor_Compra, $Valor_Venta, $Proveedores_idProveedores, $idKardex );
        echo json_encode($datos);
        break;
